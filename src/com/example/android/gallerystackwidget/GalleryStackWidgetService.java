...
public class GalleryStackWidgetService extends RemoteViewsService {
	@Override
...
class StackRemoteViewsFactory implements RemoteViewsService.RemoteViewsFactory {
	...
	public RemoteViews getViewAt(int position) {
		...
		// imgView.setImageURI(Uri.withAppendedPath(MediaStore.Images.Thumbnails.EXTERNAL_CONTENT_URI,
		// String.valueOf(1)));
		// rv.setImageViewResource(R.id.imageView1, R.drawable.cat1);
		rv.setImageViewUri(R.id.imageView1,
				Uri.withAppendedPath(MediaStore.Images.Thumbnails.EXTERNAL_CONTENT_URI, String.valueOf(position + 1)));

		// Next, we set a fill-intent which will be used to fill-in the pending
		// intent template
		// which is set on the collection view in StackWidgetProvider.
		Bundle extras = new Bundle();
		extras.putInt(GalleryStackWidgetProvider.EXTRA_ITEM, position);
